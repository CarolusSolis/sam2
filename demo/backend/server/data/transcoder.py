...
def transcode(
    ...
    max_w = int(os.environ.get("VIDEO_ENCODE_MAX_WIDTH", "3840"))
    max_h = int(os.environ.get("VIDEO_ENCODE_MAX_HEIGHT", "2160"))
...
def get_video_metadata(path: str) -> VideoMetadata:
    with av.open(path) as cont:
        ...
        if num_video_streams > 0:
            ...
            # Handle case where side_data attribute might not exist in current PyAV version
            try:
                rotation_deg = video_stream.side_data.get("DISPLAYMATRIX", 0)
            except AttributeError:
                # If side_data attribute doesn't exist, default to 0 rotation
                rotation_deg = 0
